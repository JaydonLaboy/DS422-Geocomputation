...
mapboxgl(bounds = hi_internet) |>
  ...
  add_legend(
    "Households with Internet in Hawaii",
...
```
(After trying to normalize I decided to divide by the total number of households to get a percentage.)
```{r}
library(tidycensus)
library(dplyr)
library(mapgl)
...
hi_internet <- get_acs(
  geography = "tract",
  variables = c(
    internet = "B28002_004",  # households with internet
    total    = "B28002_001"   # total households
  ),
  state = "HI",
  year = 2023,
  geometry = TRUE
...
hi_internet_wide <- hi_internet |>
  ...
  mutate(
    pct_internet = (internet / total) * 100
  )

hi_scale <- interpolate_palette(
  data = hi_internet_wide,
  column = "pct_internet",
  method = "equal",
  n = 4,
  palette = viridisLite::cividis
...
mapboxgl(bounds = hi_internet_wide) |>
  add_fill_layer(
    id = "hi_tracts",
    source = hi_internet_wide,
    fill_color = hi_scale$expression,
    fill_opacity = 0.5
  ) |>
  add_legend(
    "Percent of Households with Internet in Hawaii",
    values = get_legend_labels(hi_scale, digits = 0, suffix = "%"),
    colors = get_legend_colors(hi_scale),
    type = "continuous"
  )
