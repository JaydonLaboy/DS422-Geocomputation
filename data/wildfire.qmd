...
fires_summary <- wildfire_shp_data %>%
  mutate(ACQ_DATE = as.Date(ACQ_DATE)) %>%   
  group_by(ACQ_DATE) %>%
  summarise(fire_count = n()) %>%
  ungroup()

ggplot(fires_summary, aes(x = ACQ_DATE, y = fire_count)) +
  geom_line(color = "firebrick", linewidth = 1) +
  labs(
    title = "Number of Fires Detected per Day",
    x = "Date",
    y = "Number of Fires"
  ) +
  theme_minimal()
...
ggplot(fires_summary, aes(x = ACQ_DATE, y = fire_count, fill = CONFIDENCE)) +
  geom_bar(stat = "identity") +
  labs(
    title = "Number of Fires Detected per Day by Confidence Level",
    x = "Date",
    y = "Number of Fires",
    fill = "Confidence"
  ) +
...
wildfire_area_data <- st_read(
  "data/fire_kml/LANDSAT_USA_contiguous_and_Hawaii_7d_1757637852247.kml",
...
fires_df <- read.csv("data/LANDSAT_USA_contiguous_and_Hawaii_7d.csv")

# 2. Transform into an sf object (specify CRS = WGS84)
fires_sf <- st_as_sf(
  fires_df,
...
# 3. Map creatively
leaflet(fires_sf) |>
  addProviderTiles("CartoDB.Positron") |>  # lighter basemap for contrast
  addCircleMarkers(
    radius = ~ifelse(confidence > 80, 6, 3),   # bigger dots for higher confidence
    color = ~ifelse(daynight == "D", "orange", "blue"), # orange for day, blue for night detections
    fillOpacity = 0.7,
    popup = ~paste("Date:", acq_date,
